...
- Bundle mdx files in a `docs` folder, with [any file pattern](/configuration#options) or with a custom [`FileProvider`](/customization/DI/fileProvider).
- Support for typed [`Frontmatter`](/customization/DI/frontmatter), [`MDX`](https://mdxjs.com/) syntax in descriptions, table of contents, headings and more.
